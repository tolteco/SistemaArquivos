...
nfs_sys::nfs_sys(FILE *device, const info_t &info)
    : block_size(static_cast<unsigned int>(pow(2, info.block_size))),
      ...
      current_path("")
{
    //    cout << "Tamanho de um bloco: " << this->block_size << " bytes" << endl
    //         << "Numero de blocos por grupo: " << this->n_blocks_pg << endl
    //         << "Numero de grupos do device: " << this->n_groups << endl;
}
...
void nfs_sys::disk_to_device(const string& source) const
{
    ...
    auto current_t = current_time();
    auto name = get_file_name(source);
    auto entry = entry_t { "", static_cast<char>(name.length()),
            entry_flags::FILE |
            entry_flags::READ |
            entry_flags::WRITE,
            current_t,
...
void nfs_sys::changedir(const std::string &path)
{
