...
#pragma pack(push, 1)
    ...
    //Entrada do sistema, contém informações úteis
...
#pragma pack(pop)
    ...
    public:
        ...
        void disk_to_device(const std::string& source, const std::string& dest);
		
		// escreve do device para o disco
		// terminar de implementar
        void device_to_disk(const std::string& source, const std::string& dest) const;
        ...
        // retorna somente o nome do arquivo sem o path
        std::string get_file_name(const std::string& fullname) const;

        // mexe com a pilha de path
        void append_path(const std::string& path);

        // retorna o bloco do último diretório do path
        unsigned int get_last_dir_block(const std::string& path, bool changedir = false);

        // retorna uma entrada especificada pelo path
        const entry_t get_entry(const std::string& path) const;
    ...
    namespace Util {
        std::vector<std::string> split_string(const std::string &source, const char &delim);
    }
}
