...
nfs_sys nfs_sys::format(const std::string &device_path, const info_t &info)
{
    ...
    // informação básica de um diretório (escreve . e .. no root)
    entry_t basic[] = {
        entry_t{ ".", 1,
                 entry_flags::HIDDEN |
                 entry_flags::DIRECTORY |
...
vector<entry_t> nfs_sys::list_source() const
{
    ...
    for (auto block : read_hash(this->current_directory)) // le a lista de blocos do dir atual
    {
        fseek(this->device, block_addr(block), SEEK_SET);

        for (auto i = 0u; i < this->max_entries; i++)
        {
            fread(&entry, sizeof(entry_t), 1, this->device);    // para cada bloco le suas entradas
            if (entry.name_length <= 0) // add se nome > 0
                continue;
            arr.push_back(entry);
        }
    }
...
void nfs_sys::device_to_disk(const string& source, const string& dest) const
{
    ...
    auto entry = get_entry(source); // encontra entrada do arquivo que quer passar para o disk (source)
    ...
    for (auto block : read_hash(entry.first_block)) // le blocos da entrada
    {
        fseek(this->device, block_addr(block), SEEK_SET);
        fread(buff, sizeof(char), size >= this->block_size ? this->block_size : size, this->device);
        fwrite(buff, sizeof(char), size >= this->block_size ? this->block_size : size, dest_file);  // escreve os bytes do arquivo no destino
...
void nfs_sys::disk_to_device(const string& source, const string& dest)
{
    ...
    auto path_block = get_last_dir_block(dest.substr(0, dest.length() - file_name.length()));   // encontra a pasta que o arquivo será colocado
    ...
    auto blocks_size = size / this->block_size + (size % this->block_size ? 1 : 0); // quantos blocos serão necessários para guardar o arquivo
    auto buff = static_cast<char*>(malloc(this->block_size));
    auto blocks = free_blocks_for(blocks_size); // procura blocos livres na hash
    auto current_t = current_time();
    ...
    //g++ -std=gnu++0x -O2 NeoFS.h NeoFS.cpp teste.cpp -Wall -lrt -o foo.x

    memcpy(entry.name, file_name.c_str(), file_name.length());

    append_entry(path_block, entry);

    for (auto block : blocks)   // para cada byte do arquivo, escreve no device
    {
        fseek(this->device, block_addr(block), SEEK_SET);
    ...
    write_hash(blocks); // aloca os blocos na hash
